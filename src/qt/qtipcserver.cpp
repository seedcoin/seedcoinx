...
static bool ipcScanCmd(int argc, char *argv[], bool fRelay)
{
    ...
    {
        if (boost::algorithm::istarts_with(argv[i], "SeedcoinX:"))
        {
...
static void ipcThread(void* pArg)
{
    ...
    RenameThread("SeedcoinX-gui-ipc");
...
void ipcInit(int argc, char *argv[])
{
    ...
    try {
        ...
        // Make sure we don't lose any SeedcoinX: URIs
        for (int i = 0; i < 2; i++)
        {
        ...
        // Make sure only one SeedcoinX instance is listening
        message_queue::remove(BOUNTYCOINURI_QUEUE_NAME);
        delete mq;
