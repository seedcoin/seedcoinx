...
unsigned int nStakeTargetSpacing = .667 * 60; // 60 seconds block spacing
const int64 nChainStartTime = 1377393424; 
...
const string strMessageMagic = "SeedcoinX Signed Message:\n";
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime)
{
    ...
    // SeedcoinX: reward for coin-year is cut in half every 64x multiply of PoS difficulty
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal SeedcoinX miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in SeedcoinX
    bool fStrictPayToScriptHash = true; // Always active in SeedcoinX
    ...
    // ppcoin: fees are not collected by miners as in SeedcoinX
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "SeedcoinX", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Start Sowing start growing";
        CTransaction txNew;
        ...
        txNew.strTxComment = "text:SeedcoinX genesis block";
        ...
        assert(block.hashMerkleRoot == uint256("0x4499a5b21e35c37a7cb0becf13248c7b637e9206f4de211218eaca2679cdf2b5"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("SeedcoinXMiner : proof-of-work not meeting target");

    //// debug print
    printf("SeedcoinXMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SeedcoinXMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("SeedcoinXMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadSeedcoinXMiner(void* parg);

static bool fGenerateSeedcoinXs = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void SeedcoinXMiner(CWallet *pwallet, bool fProofOfStake)
{
    printf("CPUMiner started for proof-of-%s\n", fProofOfStake? "stake" : "work");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);

    // Make this thread recognisable as the mining thread
    RenameThread("SeedcoinX-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGenerateSeedcoinXs || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGenerateSeedcoinXs) && !fProofOfStake)
                return;
        }
        ...
        printf("Running SeedcoinXMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadSeedcoinXMiner(void* parg)
{
    ...
    try
    {
        ...
        SeedcoinXMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadSeedcoinXMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadSeedcoinXMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadSeedcoinXMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void GenerateSeedcoinXs(bool fGenerate, CWallet* pwallet)
{
    fGenerateSeedcoinXs = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateSeedcoinXs = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d SeedcoinXMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadSeedcoinXMiner, pwallet))
                printf("Error: NewThread(ThreadSeedcoinXMiner) failed\n");
            Sleep(10);
        }
    }
}
